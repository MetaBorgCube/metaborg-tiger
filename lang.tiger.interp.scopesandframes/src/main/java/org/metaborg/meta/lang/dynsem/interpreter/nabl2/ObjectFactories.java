...
public class ObjectFactories {
	...
	public static DynamicObject createNaBL2(IStrategoAppl solution, ScopesAndFramesContext ctx, TruffleLanguage<?> lang) {
		...
		DynamicObject types = createTypes(Tools.listAt(solution, 2));
		return NaBL2LayoutImpl.INSTANCE.createNaBL2(scopeGraph, nameResolution, types);
	}

	private static DynamicObject createTypes(IStrategoList typesTerm) {
		...
		for (IStrategoTerm typeEntry : typesTerm) {
			...
			types.define(decl, Ty.create(typeTerm));
		}
		return types;
	}

	private static DynamicObject createNameResolution(IStrategoList resolutionsTerm, TruffleLanguage<?> lang) {
